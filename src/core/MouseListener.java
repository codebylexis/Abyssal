...
/**
 * Singleton that tracks mouse state via GLFW callbacks. In addition to raw
 * screen coordinates it exposes world-space coordinates by unprojecting
 * through the camera's inverse view/projection matrices, accounting for the
 * game viewport's position and size within the ImGui layout.
 */
public class MouseListener {
	...
	public static Vector2f getWorld() {
		...
		Matrix4f inverseView = new Matrix4f(camera.getInverseView());
		Matrix4f inverseProjection = new Matrix4f(camera.getInverseProjection());
		tmp = tmp.mul(inverseView.mul(inverseProjection));
